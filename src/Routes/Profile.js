...
const Profile = () => {
    ...
    useEffect(()=>{
        axios.get(`http://localhost:5000/api/user/users/profile` ,{withCredentials:true}).then((res)=>setprofile(res.data.data)).catch((err)=>{ console.log(err)})
    },[])
  return (
    <Fragment>
    {
         ...
         <div class="main">
            ...
            <div id="c1" class="card">
                 <div class="card-info">
                    ...
                    <div class="card-title"> <h5> {profile.userName}</h5></div>
                    <div class="card-subtitle">{ profile.role ==='1a' ? <>  Admin</>  : (profile.role ==='2i' ? <> Instructor</> : <>Student</>)}</div>
...
<NavLink  className="a1 "to={'/home'}>
  <span className="span1">Back</span>
  <svg className="svg" width="15px" height="10px" viewBox="0 0 13 10">
    <path className="path "d="M1,5 L11,5"></path>
    <polyline className="polyline" points="8 1 12 5 8 9"></polyline>
  </svg>
</NavLink>
</button>
        </div>
