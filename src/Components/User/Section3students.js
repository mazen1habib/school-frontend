...
const Section3students = () => {
    ...
    useEffect(()=>{
        axios.get(`http://localhost:5000/api/user/students` ,{withCredentials:true}).then((res)=>setdiplomas(res.data.data)).catch((err)=>console.log(err))
    },[])
    let deleted =(id)=>{
      setErrors([])
      console.log(id)
...
<td>
<NavLink to={`/user/updategroup/${e._id}`}>
<button className={style.editBtn}>
  <svg height="1em" viewBox="0 0 512 512" className={style.svg}>
    <path
...
export default Section3students