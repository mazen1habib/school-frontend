...
const SingleUser = () => {
    ...
    useEffect(()=>{
        axios.get(`http://localhost:5000/api/user/users/single/${params.id}` ,{withCredentials:true}).then((res)=>setuser(res.data.data)).catch((err)=>console.log(err))
    },[])
  return (
    <Fragment>
    <Navorginal/>
    ...
    <Table responsive="xl" border={4} style={{width:'1000px'}} >
        ...
        <tr key={user._id} >
            ...
            <td>{user.email}</td>
            <td>{user.phone}</td>
            <td>{user.role ==='1a' ? <>Admin</> :(user.role ==='2i' ? <>Instructor</> : <>Student</>)}</td>
            {
                user.role ==='3s' && <>
                <td>{user.diploma}</td>
                <td>{user.group}</td>
            ...
            <td>{user.userCreatedBy?.name}</td>
            <td>{user.updatedAt}</td>
            <td>{user.updatedBy?.name}</td>
            <td>{user.updatedPasswordBy?.name}</td>
      ...
      <NavLink to={ user.role ==='1a' ? '/admins' : (user.role ==='2i' ? '/instructors' : '/students')}>
