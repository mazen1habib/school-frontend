...
const UpdateGroup = () => {
    ...
    const [userData,setuserData] = useState({
        ...
        let sumbitData =()=>{
            setErrors([])
            axios.patch(`http://localhost:5000/api/group/${params.id}`,userData,{withCredentials:true})
 ...
 <div  className={style.container}>
    ...
    <label className={style.label} >
        <input className={style.input} type="datetime-local" placeholder="" required="" onChange={(e)=>setuserData({...userData,startDate:e.target.value})}/>
        <span className={style.span}> New Start Date</span>
    </label> 
